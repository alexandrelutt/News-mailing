...
def save_article(filename, title, link, soup):
    f = open('/Users/alexandrelutt/Desktop/Perso/Informatique/News/dev/articles/' + filename + '.txt', 'w')
...
def get_raw_text_from(filename):
    document = open('/Users/alexandrelutt/Desktop/Perso/Informatique/News/dev/articles/' + filename + '.txt', 'r')
    text = document.read()
    document.close()
    return text
...
def save_summary(filename, summary):
    f = open('/Users/alexandrelutt/Desktop/Perso/Informatique/News/dev/summaries/' + filename + '_summarized.txt', 'w')
...
def get_links_from_main_url(url):
    ...
    for link in soup.find_all('a'):
        if 'https://www.20minutes.fr/monde/' in link.get('href'):
...
def send_email(recipient_email_adress, subject, content):
    email_address = 'easy.daily.articles@gmail.com'
    email_password = 'glrmfuhzajxekvnd'
    ...
    with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
        ...
        print('Email has been succesfully sent to', recipient_email_adress)

def get_recipient_email_adresses():
    with open('/Users/alexandrelutt/Desktop/Perso/Informatique/News/dev/short_recipients.txt') as f:
        lines = f.readlines()
